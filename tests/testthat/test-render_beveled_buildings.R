...
test_that("render_polygons() warns and combines scale_data with vertical_exaggeration", {
  ...
  polygon_ids = get_ids_with_labels()$id[
    get_ids_with_labels()$tag == "polygon3d"
  ]
  polygon_vertices = rgl::rgl.attrib(
    polygon_ids[length(polygon_ids)],
    "vertices"
  )
...
test_that("polygon data columns are coerced to numeric before dropping NA values", {
  polygon = data.frame(
    value = factor(c("10", "20")),
    bad_value = c("10", "invalid")
  )
  ...
  coerced_polygon = coerce_polygon_data_columns(
    polygon,
    data_column_top = "bad_value",
    caller = "render_polygons"
  )
  expect_equal(coerced_polygon$keep, c(TRUE, FALSE))
  expect_equal(coerced_polygon$polygon$bad_value, 10)
})
