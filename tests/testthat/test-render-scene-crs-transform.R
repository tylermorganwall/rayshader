...
test_that("plot_3d spatial scenes auto-transform sf line and polygon inputs", {
  ...
  actual_polygon_coords = actual_polygon_coords[
    order(
  ...
  expected_polygon_coords = expected_polygon_coords[
    order(
...
test_that("coord_sf faceted scenes require panel and allow explicit crs overrides", {
  skip_if_not_installed("sf")
  on.exit(rgl::close3d(), add = TRUE)
  local_rgl_use_null()
  ...
  expect_equal(sf::st_crs(line), original_crs)
})
