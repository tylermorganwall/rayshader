...
monterey_point_location_fixtures_test = function() {
  ...
  list(
    ...
    no_crs_single_sf = suppressWarnings(sf::st_set_crs(
      location_points[1, ],
      NA
    )),
    ...
    no_crs_multipoint_sfc = suppressWarnings(sf::st_set_crs(
      multipoint_sfc,
      NA
    )),
...
monterey_crs_test = function() {
  sf::st_crs(terra::crs(montereybay_spatial))
}

setup_plot3d_location_scene_test = function() {
  monterey_raster = monterey_spatial_raster_test()
  expect_no_condition(plot_3d_test(
    sphere_shade(montereybay_spatial, vertical_exaggeration = 10),
    monterey_raster,
...
expected_plot3d_location_xy_test = function(location, crs = NULL) {
  ...
  scene_points = sf::st_transform(
    rayshader:::coerce_scene_point_input(
      location,
      crs = crs,
      caller = "test"
    )$sf_data,
...
expected_plotgg_location_xy_test = function(
  location,
  crs = NULL,
  panel = NULL
) {
...
expected_plotgg_location_xy_from_explicit_crs_test = function(
  location,
  crs,
  panel = NULL
) {
...
renderer_location_cases_test = function(
  fixtures,
  scene = c("plot3d", "plotgg")
) {
...
expect_location_snapshot_test = function(
  snapshot_name,
  setup_scene,
  render_call
) {
...
plot3d_snapshot_setup_for_case_test = function(case_name) {
  setup_plot3d_location_snapshot_scene_test(
    label = identical(case_name, "render_label")
  )
}

plotgg_snapshot_setup_for_case_test = function(case_name) {
  setup_plotgg_location_scene_test(
    topdown = !identical(case_name, "render_label")
  )
}
...
test_that("CRS-less location inputs transform with explicit crs in plot_3d scenes", {
  ...
  expect_equal(
    nrow(rgl::rgl.attrib(point_id, "vertices")),
    nrow(expected_points)
  )
...
test_that("faceted coord_sf location snapshots match goldens", {
  ...
  expect_location_snapshot_test(
    ...
    setup_scene = function() {
      setup_plotgg_location_scene_faceted_test(topdown = TRUE)
    },
