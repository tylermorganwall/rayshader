test_that("Bryce GeoTIFF scenes auto-cache raster metadata for render_buildings()", {
	...
	expect_equal(get_scene_zscale(), mean(raster::res(fixtures$raster_crop)), tolerance = 1e-8)
	expect_equal(get_scene_extent(), get_extent(fixtures$raster_crop), tolerance = 1e-8)
	expect_true(scene_crs_equal(get_scene_crs(), sf::st_crs(raster::crs(fixtures$raster_crop))))
	expect_equal(dim(get_scene_heightmap()), dim(fixtures$heightmap))

	sample_rows = unique(round(c(1, nrow(fixtures$heightmap) / 2, nrow(fixtures$heightmap))))
	sample_cols = unique(round(c(1, ncol(fixtures$heightmap) / 2, ncol(fixtures$heightmap))))
	expect_equal(
