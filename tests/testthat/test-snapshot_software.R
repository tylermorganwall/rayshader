...
test_that("Checking render_snapshot(software_render = TRUE) features", {
  ...
  # render_label(montereybay_spatial,lat = santa_cruz[1]+0.1, long = santa_cruz[2],
  #              altitude=2000, text = "Santa Cruz 2", clear_previous = T)
  # render_label(montereybay_spatial,lat = santa_cruz[1], long = santa_cruz[2],
  ...
  software_render_args = expand.grid(
    fsaa = list(1, 2),
  ...
  run_tests_sw(
    "render_snapshot",
    software_render_args,
    plot_prefix = "sw_render",
    list(cache_scene = TRUE, software_render = TRUE)
  )

  software_render_args_dims = expand.grid(
    software_render = list(TRUE),
    width = list(NULL, 600),
    height = list(NULL, 600)
  )
  run_tests_sw(
    "render_snapshot",
    software_render_args_dims,
    plot_prefix = "sw_render_dims",
    list(cache_scene = TRUE)
  )

  software_render_args_lines = expand.grid(
    software_render = list(TRUE),
  ...
  run_tests_sw(
    "render_snapshot",
    software_render_args_lines,
    plot_prefix = "sw_render_lines",
    list(cache_scene = TRUE)
  )
  ...
  software_render_points_args = expand.grid(
    software_render = list(TRUE),
  ...
  run_tests_sw(
    "render_snapshot",
    software_render_points_args,
    plot_prefix = "sw_render_points",
    list(cache_scene = TRUE)
  )
  rgl::close3d()
})
