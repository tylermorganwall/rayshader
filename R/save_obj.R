...
save_obj = function(
		...
		if (save_texture) {
			...
			if (tools::file_ext(filename) == "obj") {
				if (current_dir) {
					...
					filename_mtl = sprintf(
						"%s%s%s.mtl",
						dirname(filename),
						.Platform$file.sep,
						tools::file_path_sans_ext(tempbase)
					)
				}
			} else {
				if (current_dir) {
	...
	write_data = function(id, con) {
		...
		if (nrow(textures) > 0) {
			cat(
				paste0("vt ", sprintf("%1.6f %1.6f", textures[, 1], textures[, 2])),
				file = con,
				sep = "\n"
			)
		...
		if (nrow(normals) > 0) {
			cat(
				paste0(
					"vn ",
					sprintf("%1.6f %1.6f %1.6f", normals[, 1], normals[, 2], normals[, 3])
				),
				file = con,
				sep = "\n"
			)
	...
	write_mtl = function(idrow, con) {
		if (!is.na(idrow$texture_file) && is.na(idrow$obj_color)) {
			...
			file.copy(
				idrow$texture_file[[1]],
				sprintf("%s.png", noext_filename),
				overwrite = TRUE
			)
			...
			cat(
				paste(
					"Kd",
					sprintf("%1.4f %1.4f %1.4f", tempcol[1], tempcol[2], tempcol[3]),
					collapse = " "
				),
				file = con,
				sep = "\n"
			...
			cat(
				paste("d", sprintf("%1.4f", idrow$water_alpha[[1]]), collapse = " "),
				file = con,
				sep = "\n"
			)
			cat(
				paste("Ni", sprintf("%1.4f", water_index_refraction), collapse = " "),
				file = con,
				sep = "\n"
			)
			cat("illum 5 \n", file = con)
			...
			cat(
				sprintf("newmtl ray_north%d", current_compass_number),
				file = con,
				sep = "\n"
			)
			cat(
			...
			cat(
				sprintf("newmtl ray_arrow%d", current_compass_number),
				file = con,
				sep = "\n"
			)
			cat(
			...
			cat(
				sprintf("newmtl ray_bevel%d", current_compass_number),
				file = con,
				sep = "\n"
			)
			cat(
			...
			cat(
				sprintf("newmtl ray_background%d", current_compass_number),
				file = con,
				sep = "\n"
			)
			cat(
			...
			cat(
				paste("d", sprintf("%1.4f", idrow$polygon_alpha[[1]]), collapse = " "),
				file = con,
				sep = "\n"
			)
			if (idrow$lit[[1]]) {
			...
			if (save_shadow) {
				...
				file.copy(
					idrow$shadow_texture_file[[1]],
					sprintf("%s_shadow.png", noext_filename),
					overwrite = TRUE
				)
				cat(
					sprintf(
						"map_Kd %s \n",
						sprintf("%s_shadow.png", basename(noext_filename))
					),
					file = con
				)
				cat("\n", file = con)
			...
			cat(
				sprintf("newmtl ray_layer_texture%d \n", floating_layer_num),
				file = con
			)
			file.copy(
				idrow$layer_texture_file[[1]],
				sprintf("%s_layer%d.png", noext_filename, floating_layer_num),
				overwrite = TRUE
			)
			cat(
				sprintf(
			...
			file.copy(
				idrow$soil_texture[[1]],
				sprintf("%s_soil%d.png", noext_filename, soil_textures),
				overwrite = TRUE
			)
			cat(
				sprintf(
					"map_Kd %s \n",
					sprintf("%s_soil%d.png", basename(noext_filename), soil_textures)
				),
				file = con
			)
			...
			if (
				!is.na(idrow$texture_file[[1]]) && file.exists(idrow$texture_file[[1]])
			) {
				file.copy(
					idrow$texture_file[[1]],
					sprintf("%s_obj%d.png", noext_filename, obj_materials),
					overwrite = TRUE
				)
				cat(
					sprintf(
						"map_Kd %s \n",
						sprintf("%s_obj%d.png", basename(noext_filename), obj_materials)
					),
					file = con
				)
			}
			...
			cat(
				paste("d", sprintf("%1.4f", tempalpha), collapse = " "),
				file = con,
				sep = "\n"
			)
		...
		if (
			...
			if (
				vertex_info$tag[row] %in%
					...
					grepl("obj", vertex_info$tag[row], fixed = TRUE)
			) {
	...
	for (row in seq_len(nrow(vertex_info))) {
		if (
			...
			if (
				...
				hasnormals = vertex_info$startindexnormals[row] <
					vertex_info$endindexnormals[row]
			} else {
				hasnormals = FALSE
			}
			nx = dims[1]
			...
			for (i in seq_len(nz)[-nz]) {
				for (j in seq_len(nx)[-nx]) {
					if (
						!is.na(vertices_y[(i - 1) * nx + j]) &&
							!is.na(vertices_y[(i + 1 - 1) * nx + j]) &&
							!is.na(vertices_y[(i - 1) * nx + j + 1]) &&
							!is.na(vertices_y[(i + 1 - 1) * nx + j + 1])
					) {
						cindices = (i - 1) *
							nx +
							c(j, j + nx, j + 1, j + nx, j + nx + 1, j + 1)
			...
			cat(
				paste("f", indices[, 1], indices[, 2], indices[, 3]),
				sep = "\n",
				file = con
			)
			...
			indices = matrix(
				rgl::rgl.attrib(vertex_info$id[row], "indices"),
				ncol = 3L
			) +
				vertex_info$startindex[row] -
				1
			if (hasnormals) {
				indices = sprintf("%d/%d/%d", indices, indices, indices)
			} else {
			...
			indices = indices[1:(nrow(indices) - na_counter), ]
			cat(
				paste("f", indices[, 3], indices[, 2], indices[, 1]),
				sep = "\n",
				file = con
			)
			...
			baseindices = matrix(
				vertex_info$startindex[row]:vertex_info$endindex[row],
				ncol = 3,
				byrow = TRUE
			)
			# if(hasnormals) {
			...
			cat(
				paste(
					"f",
					sprintf("%d %d %d", indices[, 1], indices[, 2], indices[, 3])
				),
				sep = "\n",
				file = con
			)
			...
			if (save_texture) {
				...
				cat(
					sprintf("usemtl ray_north%d", current_compass_number),
					file = con,
					sep = "\n"
				)
			}
			...
			cat(
				paste("f", baseindices[, 1], baseindices[, 2], baseindices[, 3]),
				sep = "\n",
				file = con
			)
			...
			if (save_texture) {
				...
				cat(
					sprintf("usemtl ray_arrow%d", current_compass_number),
					file = con,
					sep = "\n"
				)
			}
			...
			if (save_texture) {
				...
				cat(
					sprintf("usemtl ray_bevel%d", current_compass_number),
					file = con,
					sep = "\n"
				)
			}
			...
			if (save_texture) {
				...
				cat(
					sprintf("usemtl ray_background%d", current_compass_number),
					file = con,
					sep = "\n"
				)
			}
			...
			shadowindices = matrix(
				vertex_info$startindex[row]:vertex_info$endindex[row],
				ncol = 3,
				byrow = TRUE
			)
			shadowtexindices = matrix(
				vertex_info$startindextex[row]:vertex_info$endindextex[row],
				ncol = 3,
				byrow = TRUE
			)
			shadownormalindices = matrix(
				vertex_info$startindexnormals[row]:vertex_info$endindexnormals[row],
				ncol = 3,
				byrow = TRUE
			)
			...
			cat(
				paste(
					...
					sprintf(
						...
						shadowindices[, 1],
						shadowtexindices[, 1],
						shadownormalindices[, 1],
						shadowindices[, 2],
						shadowtexindices[, 2],
						shadownormalindices[, 2],
						shadowindices[, 3],
						shadowtexindices[, 3],
						shadownormalindices[, 3]
					)
				),
				sep = "\n",
				file = con
			...
			vertex_info$tag[row] %in% c("floating_overlay", "floating_overlay_tris")
		) {
			if (save_texture) {
				cat("g Floating", file = con, sep = "\n")
				cat(
					sprintf("usemtl ray_layer_texture%d", current_floating_number),
					file = con,
					sep = "\n"
				)
				current_floating_number = current_floating_number + 1
			}
			floating_layer_indices = matrix(
				vertex_info$startindex[row]:vertex_info$endindex[row],
				ncol = 3,
				byrow = TRUE
			)
			floating_layer_texindices = matrix(
				vertex_info$startindextex[row]:vertex_info$endindextex[row],
				ncol = 3,
				byrow = TRUE
			)
			floating_layer_normalindices = matrix(
				vertex_info$startindexnormals[row]:vertex_info$startindexnormals[row],
				ncol = 3,
				byrow = TRUE
			)
			cat(
				paste(
					...
					sprintf(
						"%d/%d/%d %d/%d/%d %d/%d/%d",
						floating_layer_indices[, 1],
						floating_layer_texindices[, 1],
						floating_layer_normalindices[, 1],
						floating_layer_indices[, 2],
						floating_layer_texindices[, 2],
						floating_layer_normalindices[, 2],
						floating_layer_indices[, 3],
						floating_layer_texindices[, 3],
						floating_layer_normalindices[, 3]
					)
				),
				sep = "\n",
				file = con
			...
			soil_layer_indices = matrix(
				vertex_info$startindex[row]:vertex_info$endindex[row],
				ncol = 3,
				byrow = TRUE
			)
			soil_layer_texindices = matrix(
				vertex_info$startindextex[row]:vertex_info$endindextex[row],
				ncol = 3,
				byrow = TRUE
			)
			...
			cat(
				paste(
					...
					sprintf(
						"%d/%d %d/%d %d/%d",
						soil_layer_indices[, 1],
						soil_layer_texindices[, 1], #soil_layer_normalindices[,1],
						soil_layer_indices[, 2],
						soil_layer_texindices[, 2], #soil_layer_normalindices[,2],
						soil_layer_indices[, 3],
						soil_layer_texindices[, 3]
					)
				), #soil_layer_normalindices[,3])),
				sep = "\n",
				file = con
			)
			...
			if (save_texture) {
				...
				cat(
					sprintf("usemtl ray_obj%d", current_obj_number),
					file = con,
					sep = "\n"
				)
			...
			obj_indices = matrix(
				inds + vertex_info$startindex[row],
				byrow = TRUE,
				ncol = 3
			)
			obj_tex_indices = matrix(
				inds + vertex_info$startindextex[row],
				byrow = TRUE,
				ncol = 3
			)
			obj_norm_indices = matrix(
				inds + vertex_info$startindexnormals[row],
				byrow = TRUE,
				ncol = 3
			)
