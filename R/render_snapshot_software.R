...
render_snapshot_software = function(
    ...
    for (j in seq_len(nrow(temp_label))) {
      ...
      labels = rayvertex::add_shape(
        ...
        rayvertex::text3d_mesh(
          ...
          position = c(
            temp_center[j, 1] - bbox_center[1] + text_offset[1],
  ...
  labellineids = get_ids_with_labels(
    typeval = c("textline", "lines", "waterlines")
  )$id
  ...
  for (i in seq_len(length(labellineids))) {
    ...
    if (thick_lines) {
      for (j in seq_len(nrow(temp_verts) / 2)) {
        line_mat = rayvertex::material_list(
          diffuse = temp_color[j, 1:3],
          type = "color"
        )
        line_scene[[line_counter]] = rayvertex::segment_mesh(
      ...
      for (j in seq_len(nrow(temp_verts) / 2)) {
        labelline = rayvertex::add_lines(
          labelline,
          rayvertex::generate_line(
            start = temp_verts[2 * j - 1, ] - bbox_center,
            end = temp_verts[2 * j, ] - bbox_center,
    ...
    if (thick_lines) {
      for (j in seq_len(nrow(temp_verts) - 1)) {
        ...
        if (!all(temp_verts[j + 1, ] == temp_verts[j, ])) {
          line_scene[[line_counter]] = rayvertex::segment_mesh(
            start = temp_verts[j, ] - bbox_center,
  ...
  for (i in seq_len(length(pointids))) {
    ...
    for (j in seq_len(nrow(temp_verts))) {
      pointlist = rayvertex::add_shape(
        pointlist,
        rayvertex::sphere_mesh(
          position = c(
            temp_verts[j, 1] - bbox_center[1],
  ...
  for (i in seq_len(length(scalelabelids))) {
    ...
    for (j in seq_len(nrow(temp_label))) {
      scalelabels = rayvertex::add_shape(
        scalelabels,
        rayvertex::text3d_mesh(
          as.character(temp_label),
          position = c(
            temp_center[j, 1] - bbox_center[1] + text_offset[1],
            temp_center[j, 2] - bbox_center[2] + text_offset[2],
            temp_center[j, 3] - bbox_center[3] + text_offset[3]
  ...
  if (print_scene_info) {
    print(sprintf(
      "Camera position: c(%0.2f, %0.2f, %0.2f), Camera Lookat: c(%0.2f, %0.2f, %0.2f)",
      lookfrom[1],
      lookfrom[2],
      lookfrom[3],
      camera_lookat[1],
    ...
    if (!is.null(scene$materials$ray_shadow)) {
      if (file.exists(scene$materials$ray_shadow$diffuse_texname)) {
        ...
        file.exists(sprintf(
          ...
          tempdir(),
          sepval,
          scene$materials$ray_shadow$diffuse_texname
        ))
      ) {
        shdw = png::readPNG(sprintf(
          "%s%s%s",
          tempdir(),
          sepval,
          scene$materials$ray_shadow$diffuse_texname
