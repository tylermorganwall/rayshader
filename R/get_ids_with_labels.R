...
get_ids_with_labels = function(typeval = NULL) {
  ...
  for (i in seq_len(nrow(idvals))) {
    ...
    if (idvals$type[i] != "text") {
      if (substr(material_type[i], 1, 7) == "surface") {
        full_texture = get_plot_3d_surface_texture(
          idvals$id[i],
          material_type_single$texture
        )
        if (is.null(full_texture)) {
          full_texture = material_type_single$texture
        }
        material_properties[[i]]$texture_file = full_texture
      ...
      if (
        material_type[i] %in% c("floating_overlay", "floating_overlay_tris")
      ) {
        material_properties[[
          i
        ]]$layer_texture_file = material_type_single$texture
      }
      if (material_type[i] %in% c("base", "basebottom")) {
      ...
      if (material_type[i] == "shadow") {
        material_properties[[
          i
        ]]$shadow_texture_file = material_type_single$texture
      }
  ...
  if (!is.null(typeval)) {
    is_surface_tri = substr(retval$tag, 1, 12) == "surface_tris" &
      any(typeval == "surface_tris")
    is_surface = (substr(retval$tag, 1, 7) == "surface") &
      !is_surface_tri &
      any(typeval == "surface")
    is_obj = substr(material_type, 1, 3) == "obj" & any(typeval == "obj")

    if (
      any(typeval %in% ray_types) ||
        any(is_surface) ||
        any(is_surface_tri) ||
        any(is_obj)
    ) {
      retval = retval[
        retval$tag %in% typeval | is_surface | is_surface_tri | is_obj,
      ]
    }
  }
  return(retval)
