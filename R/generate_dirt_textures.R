...
#'@keywords internal
generate_soil_textures = function(
	heightmap,
	base_depth = 10,
	color1 = "#7d6f5b",
	color2 = "#3b3020",
	...
	soil_array1 = height_shade(
		ambient::noise_perlin(
			dim = c(nr, floor(full_range) * asp),
			frequency = freq,
			octaves = levels
		),
		texture = grDevices::colorRampPalette(c(color1, color2))(256)
	)
	soil_array2 = height_shade(
		ambient::noise_perlin(
			dim = c(nc, floor(full_range) * asp),
			frequency = freq,
			octaves = levels
		),
		texture = grDevices::colorRampPalette(c(color1, color2))(256)
	...
	if (soil_gradient > 0) {
		color2 = convert_color(
			darken_color(color2, darken = 1 / gradient_darken),
			linear = TRUE
		)
		...
		grad_soil1[,, 4] = matrix(
			1 - seq(0, 1, length.out = nr)^(soil_gradient),
			nrow = nr,
			ncol = floor(full_range) * asp
		)

		soil_array1 = rayimage::render_image_overlay(soil_array1, grad_soil1)
		...
		grad_soil2[,, 4] = matrix(
			1 - seq(0, 1, length.out = nc)^(soil_gradient),
			nrow = nc,
			ncol = floor(full_range) * asp
		)
	...
	rayimage::ray_write_image(soil_array1, tempfile1)
	rayimage::ray_write_image(soil_array2, tempfile2)
