...
transform_into_heightmap_coords = function(
  ...
  distances_y = 1 +
    ncol_map -
    (lat - e["ymin"]) / (e["ymax"] - e["ymin"]) * ncol_map
  ...
  if (is.null(altitude)) {
    ...
    distances_x_index[floor(distances_x_index) >= nrow(heightmap)] = nrow(
      heightmap
    )
    distances_y_index[floor(distances_y_index) >= ncol(heightmap)] = ncol(
      heightmap
    )
    ...
    if (!length(find.package("rayimage", quiet = TRUE)) > 0) {
      xy = matrix(
        c(floor(distances_x_index), floor(distances_y_index)),
        nrow = length(distances_x_index),
        ncol = 2
      )
      flipped_mat = flipud(t(heightmap))
      altitude = apply(xy, 1, (function(x) flipped_mat[x[2], x[1]]))
    } else {
      altitude = rayimage::interpolate_array(
        (t(heightmap)),
        distances_x_index,
        distances_y_index
      )
  ...
  if (use_altitude) {
    matrix_vals = (matrix(
      c(
        ...
        altitude / zscale + offset,
        distances_y - ncol_map / 2 - 1
      ),
      ncol = 3,
      nrow = length(altitude)
    ))
  } else {
    matrix_vals = (matrix(
      c(distances_x - nrow_map / 2 - 1, offset, distances_y - ncol_map / 2 - 1),
      ncol = 3,
      nrow = length(altitude)
    ))
  }
  missing_height = is.na(matrix_vals[, 2])
