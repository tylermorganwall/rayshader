...
#'
#'@description Adds 3D raymesh model to the current scene, using x/y coordinates in the reference
...
#'
#'If no x/y coordinates are passed in, the raymesh will be plotted in the coordinate system set by the user-specified
...
render_raymesh = function(
  ...
  for (j in seq_len(number_materials)) {
    ...
    if (nchar(texture) == 0) {
      texture = NULL
      has_texture = FALSE
      diffuse_col = convert_color(
        darken_color(
          obj$materials[[j]]$diffuse,
          obj$materials[[j]]$diffuse_intensity
        ),
        as_hex = TRUE
      )
      specular_col = convert_color(
        darken_color(
          obj$materials[[j]]$specular,
          obj$materials[[j]]$specular_intensity
        ),
        as_hex = TRUE
      )
      ambient_col = convert_color(
        darken_color(
          obj$materials[[j]]$ambient,
          obj$materials[[j]]$ambient_intensity
        ),
        as_hex = TRUE
      )
    }
