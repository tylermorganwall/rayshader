...
render_depth = function(
  ...
  if (!software_render) {
    ...
    all_image = render_snapshot_software(
      filename = temp,
  ...
  if (preview_focus) {
    ...
    tempmap2 = rayimage::render_bokeh(
      tempmap,
      depthmap = depthmap,
      focus = focus,
    ...
    if (!is.null(image_overlay)) {
      if (inherits(image_overlay, "character")) {
        ...
        rayimage::ray_write_image(image_overlay, image_overlay_file)
        has_overlay = TRUE
      }
    } else {
      has_overlay = FALSE
    }
    dimensions = dim(tempmap2)
    if (has_overlay) {
      tempmap2 = rayimage::render_image_overlay(
        tempmap2,
        image_overlay = image_overlay_file
      )
    }
    if (vignette || is.numeric(vignette)) {
      tempmap2 = rayimage::render_vignette(
        tempmap2,
        vignette = vignette,
    ...
    if (has_title) {
      tempmap2 = rayimage::render_title(
        tempmap2,
