...
#'`rayrender::screen_text()` for screen-space labels. Per-label arguments generated by rayshader
#'(e.g. label, point, size, color, justification, and occlusion) take precedence.
...
#'`rayrender::screen_line()` for screen-space lines. Per-line arguments generated by rayshader
#'(e.g. endpoints, width, color, alpha, and occlusion) take precedence.
    ...
    for (j in seq_len(nrow(temp_label))) {
      ...
      if (identical(text_render, "screen")) {
        ...
        labels_screen[[screen_counter]] = make_render_highquality_screen_text(
          ...
          hjust = get_render_highquality_screen_text_just(temp_adj, 1),
          vjust = get_render_highquality_screen_text_just(temp_adj, 2),
...
get_render_highquality_screen_text_just = function(adj, index) {
  if (is.null(adj)) {
...
make_render_highquality_screen_text = function(screen_text_args, ...) {
  do.call(
    rayrender::screen_text,
    utils::modifyList(screen_text_args, list(...))
  )
}
...
make_render_highquality_screen_line = function(screen_line_args, ...) {
  do.call(
    rayrender::screen_line,
    utils::modifyList(screen_line_args, list(...))
  )
}
