#'@title Render GIF
#'
#'@description Renders a gif using the \pkg{gifski} package. Moves the camera around a 3D visualization 
...
#'rgl::rgl.close()
#'}
render_gif = function(filename, type = "orbit", frames = 360, fps = 30, 
  ...
  gifski::gifski(png_files = png_files, gif_file = filename,
                 height = dimensions[1], width = dimensions[2],
                 delay = 1 / fps, loop = loop, progress = progbar)
}
