...
make_base = function(
  ...
  if (
    all(!is.na(heightmap)) && length(edge_vals) == 1 && all(!is.na(edge_vals))
  ) {
    heightlist = list()
    heightlist[[1]] = matrix(
      c(1, nr, nr, basedepth, basedepth, basedepth, 1, nc, 1),
      3,
      3
    )
    heightlist[[2]] = matrix(
      c(1, nr, 1, basedepth, basedepth, basedepth, nc, nc, 1),
      3,
      3
    )
    heightlist[[3]] = matrix(
      c(1, nr, 1, basedepth, basedepth, edge_vals, 1, 1, 1),
      3,
      3
    )
    heightlist[[4]] = matrix(
      c(nr, nr, 1, basedepth, edge_vals, edge_vals, 1, 1, 1),
      3,
      3
    )
    heightlist[[5]] = matrix(
      c(1, nr, 1, edge_vals, basedepth, basedepth, nc, nc, nc),
      3,
      3
    )
    heightlist[[6]] = matrix(
      c(1, nr, nr, edge_vals, edge_vals, basedepth, nc, nc, nc),
      3,
      3
    )
    heightlist[[7]] = matrix(
      c(nr, nr, nr, basedepth, basedepth, edge_vals, 1, nc, 1),
      3,
      3
    )
    heightlist[[8]] = matrix(
      c(nr, nr, nr, basedepth, edge_vals, edge_vals, nc, nc, 1),
      3,
      3
    )
    heightlist[[9]] = matrix(
      c(1, 1, 1, edge_vals, basedepth, basedepth, 1, nc, 1),
      3,
      3
    )
    heightlist[[10]] = matrix(
      c(1, 1, 1, edge_vals, edge_vals, basedepth, 1, nc, nc),
      3,
      3
    )
    ...
    edge_heights = as.vector(t(cbind(
      baselist$edge_heights,
      baselist$edge_heights,
      baselist$edge_heights
    )))
    direction_vec = as.vector(t(cbind(
      baselist$is_horizontal,
      baselist$is_horizontal,
      baselist$is_horizontal
    )))

    heightlist[[length(heightlist) + 1]] = matrix(
      c(1, nr, nr, basedepth, basedepth, basedepth, -1, -nc, -1),
      3,
      3
    )
    ...
    if (soil) {
      ...
      textures = generate_soil_textures(
        heightmap,
        base_depth = max(fullsides[, 2], na.rm = TRUE) - basedepth,
        zscale = zscale,
        freq = soil_freq,
        levels = soil_levels,
        color1 = soil_color1,
        ...
        gradient_darken = gradient_darken
      )
    ...
    baselist = make_base_cpp(heightmap, na_matrix, basedepth)
    heightlist = baselist$vertices
    edge_heights = rev(as.vector(t(cbind(
      baselist$edge_heights,
      baselist$edge_heights,
      baselist$edge_heights
    ))))

    direction_vec = rev(as.vector(t(cbind(
      baselist$is_horizontal,
      baselist$is_horizontal,
      baselist$is_horizontal
    ))))
    ...
    if (soil) {
      ...
      horizontal_texcoords_y = rep(0, length(horizontal_sides[, 2]))
      horizontal_texcoords_y[
        horizontal_sides[, 2] == basedepth
      ] = horizontal_heights[horizontal_sides[, 2] == basedepth] /
        (max_hside_y - min_hside_y)
      ...
      # vertical_texcoords_y = (vertical_sides[,2]-min_vside_y)/(max_vside_y-min_vside_y)
      vertical_texcoords_y = rep(0, length(vertical_sides[, 2]))
      vertical_texcoords_y[vertical_sides[, 2] == basedepth] = vertical_heights[
        vertical_sides[, 2] == basedepth
      ] /
