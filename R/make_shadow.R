...
make_shadow = function(
	...
	basedepthmat = matrix(
		basedepth,
		nrow = rows + shadowwidth * 2,
		ncol = cols + shadowwidth * 2
	)
	na_depth = matrix(
		FALSE,
		nrow = rows + shadowwidth * 2,
	...
	rmat[fliplr(na_depth)] = colors[1]
	gmat[fliplr(na_depth)] = colors[2]
	bmat[fliplr(na_depth)] = colors[3]
	shadowarray = array(
		1,
		dim = c(cols + shadowwidth * 2, rows + shadowwidth * 2, 3)
	)
	...
	if (has_magick) {
		...
		rayimage::render_convolution_fft(
			shadowarray,
			filename = tempmap,
			kernel = rayimage::generate_2d_gaussian(
				dim = c(shadowwidth / 2, shadowwidth / 2)
			)
		)
	} else {
		warning(
			"`magick` or `rayimage` package required for smooth shadow--using basic shadow instead."
		)
		rayimage::ray_write_image(shadowarray, tempmap)
	}
