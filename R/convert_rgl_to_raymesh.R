...
convert_rgl_to_raymesh = function(
	...
	for (row in seq_len(nrow(vertex_info))) {
		...
		if (
			...
			texture_loc = ifelse(!is.na(texture_loc), texture_loc, "")
			final_scene[[num_elems]] = rayvertex::construct_mesh(
				indices = indices,
				vertices = vertices,
				texcoords = textures,
				tex_indices = indices,
			...
			final_scene[[num_elems]] = basic_load_mesh(
				row,
				texture_loc = vertex_info$texture_file[[row]]
			)
			...
			shininess = vertex_info$shininess[[row]]
			tmp_obj = rayvertex::change_material(
				tmp_obj,
