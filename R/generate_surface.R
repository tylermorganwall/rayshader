...
#' @param geographic_aspect Default `get_scene_geographic_aspect()`. Geographic
#' aspect metadata used to scale the horizontal axes.
#'@keywords internal
generate_surface = function(
  ...
  verts = apply_geographic_aspect_to_vertices(
    verts,
    aspect = geographic_aspect
  )

  #Each surface index associated with two triangles
  offset_row_top = matrix(
    c(0L, nr, 1L),
    ncol = 3L,
    nrow = (nr - 1L) * (nc - 1L),
    byrow = TRUE
  )
  offset_row_bot = matrix(
    c(1L, nr, nr + 1L),
    ncol = 3L,
    nrow = (nr - 1L) * (nc - 1L),
    byrow = TRUE
  ...
  indices_all = matrix(seq_len(length(heightmap)), nrow = nr, ncol = nc)
  ...
  interleved_triangles = matrix(
    c(t(cbind(indices_top_triangle, indices_bot_triangle))),
    nrow = 3L
  )
