...
elmat |>
	sphere_shade(texture = "desert", vertical_exaggeration = 30) |>
	plot_map()

#sphere_shade can shift the sun direction:
elmat |>
	sphere_shade(sunangle = 45, texture = "desert", vertical_exaggeration = 30) |>
	plot_map()
...
elmat |>
	sphere_shade(texture = "desert", vertical_exaggeration = 30) |>
	add_water(detect_water(), color = "desert") |>
	add_shadow(ray_shade(vertical_exaggeration = 4), 0.25) |>
	add_shadow(ambient_shade(vertical_exaggeration = 200, maxsearch = 20), 0) |>
	plot_map()
```
...
montereybay_spatial |>
	...
	add_shadow(ray_shade(vertical_exaggeration = 4, lambert = FALSE), 0) |>
...
For a real hydrology scene, we can combine polygon water bodies, stream linework,
and a DEM from public data sources. Here, `tigris` downloads TIGER/Line water
features for Mono County, California, and `elevatr` downloads an AWS terrain
raster for the Mammoth Lakes, June Lake, and Crowley Lake region. We transform
everything to UTM zone 11N (`EPSG:32611`) so that areas, lengths, and stream
widths are in meter-like projected units before cropping to the scene.

The polygon water features are filtered to lakes and reservoirs, then each
water body is assigned a representative elevation from the DEM. Rasterizing
those elevations gives `render_water()` a spatially varying water surface,
while `render_streams()` draws the TIGER/Line stream network on the terrain.
For the final path trace, `joined_stream_mesh = TRUE` renders connected stream
branches as joined water meshes instead of separate capped line segments.

```{r README_streams-mammoth, eval = FALSE}
library(sf)
library(terra)
library(dplyr)
library(tigris)
